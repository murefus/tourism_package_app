...
mlflow.set_experiment("Tourism_Package_Prediction") # set the MLflow experiment name (same as the dev experimentation cell)

# Xtrain/Xtest/ytrain/ytest are downloaded as artifacts to the current directory
Xtrain = pd.read_csv("Xtrain.csv")
Xtest = pd.read_csv("Xtest.csv")
ytrain = pd.read_csv("ytrain.csv").squeeze()
ytest = pd.read_csv("ytest.csv").squeeze()

numeric_features = ['Age', 'CityTier', 'DurationOfPitch', 'NumberOfPersonVisiting', 'NumberOfFollowups', 'PreferredPropertyStar', 'NumberOfTrips', 'Passport', 'PitchSatisfactionScore', 'OwnCar', 'NumberOfChildrenVisiting', 'MonthlyIncome']   # list all numerical feature names (same as in prep.py)

categorical_features = ['TypeofContact', 'Occupation', 'Gender', 'ProductPitched', 'MaritalStatus', 'Designation']   # list all categorical feature names (same as in prep.py)
...
with mlflow.start_run():
    ...
    # Save the model to tourism_project/deployment/model.joblib for git commit and Streamlit app
